...
def main(dataset: ModelParams, pipe: PipelineParams, args):
    ...
    base_cam = train_cameras[0] # 获取基准相机的 FOV，防止矩阵爆炸
    ...
    def _(client: viser.ClientHandle):
        ...
        client.camera.look_at = (0.0, 0.0, 0.0) # 聚焦原点，操作不反转
    ...
    while True:
        ...
        for client_id, client in clients.items():
            ...
            view_cam = MiniCam(RENDER_SIZE, RENDER_SIZE, fovy, fovx, 0.01, 1000.0, world_view_transform, full_proj_transform)
            ...
            # 🌟 黑边填充大法 (防拉伸，保持完美 1:1)
            # ==========================================
            browser_aspect = cam_state.aspect # 浏览器的宽高比
            
            if browser_aspect > 1.0: # 浏览器比渲染图宽 (加左右黑边)
                canvas_H = RENDER_SIZE
                canvas_W = int(RENDER_SIZE * browser_aspect)
            else: # 浏览器比渲染图高 (加上下黑边)
                canvas_W = RENDER_SIZE
                canvas_H = int(RENDER_SIZE / browser_aspect)
                
            # 创建黑色背景画布
            canvas = np.zeros((canvas_H, canvas_W, 3), dtype=np.uint8)
            
            # 把渲染的 Miku 贴在画布正中间
            start_y = (canvas_H - RENDER_SIZE) // 2
            start_x = (canvas_W - RENDER_SIZE) // 2
            canvas[start_y:start_y+RENDER_SIZE, start_x:start_x+RENDER_SIZE] = img_np
            
            # 发送带黑边的画布给网页
            client.scene.set_background_image(canvas, format="jpeg")
            
        time.sleep(0.02) # 50fps 刷新率
