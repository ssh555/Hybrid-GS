    ...
    def train_phase1_window(self, win_idx, start_frame, end_frame):
        ...
        for iteration in range(1, total_iters + 1):
            ...
            for batch_idx in range(batch_size):
                ...
                # --- 软约束 (解冻后生效) ---
                total_reg_loss = 0.0  
                if self.use_soft and iteration < (self.opt.soft_end * self.iterations):
            ...
            with torch.no_grad():
                if iteration < self.densify_until_iter and (self.opt.densify_until_num_points < 0 or (self.gaussians.get_xyz.shape[0] + (self.gaussians.get_static_xyz.shape[0] if static else 0)) < self.opt.densify_until_num_points):
                    ...
                    if iteration % self.opt.opacity_reset_interval == 0 or (self.dataset.white_background and iteration == self.densify_from_iter) or (win_idx > self.opt.freeze_end_idx and iteration == self.densify_until_iter // 2):
