...
dataset=('miku_shaungxue_daxi_single_nowide' 'miku_shaungxue_daxi_single_wide' 'Illussion_XingKong_single_nowide' 'Illussion_XingKong_single_wide' 'miku_shuangxue_daxi_more_nowide' 'miku_shuangxue_daxi_more_wide' 'miku_shuangxue_daxi_more_wide' 'Illussion_XingKong_more_nowide')
...
# Hybrid-GS SHORT Hard
for data in ${dataset[@]};
do
    model_path=output/hybrid_gs/${data}/hard_${short}
    source_path=datasets/${data}/${short}
    # 在yaml配置文件中恢复了model_path和source_path的默认值，因此这里不需要再传入参数了。
    CUDA_VISIBLE_DEVICES=${device} python3 train.py --config configs/${config}/HybridGS_${Short}_Hard.yaml  --model_path $model_path \
    --source_path $source_path 
    CUDA_VISIBLE_DEVICES=${device} python3 render.py --config configs/${config}/HybridGS_${Short}_Hard.yaml  --model_path $model_path \
    --source_path $source_path --start_checkpoint $model_path/chkpnt_8000.pth
done

# Hybrid-GS SHORT Soft
for data in ${dataset[@]};
do
    model_path=output/hybrid_gs/${data}/soft_${short}
    source_path=datasets/${data}/${short}
    # 在yaml配置文件中恢复了model_path和source_path的默认值，因此这里不需要再传入参数了。
    CUDA_VISIBLE_DEVICES=${device} python3 train.py --config configs/${config}/HybridGS_${Short}_Soft.yaml  --model_path $model_path \
    --source_path $source_path 
    CUDA_VISIBLE_DEVICES=${device} python3 render.py --config configs/${config}/HybridGS_${Short}_Soft.yaml  --model_path $model_path \
    --source_path $source_path --start_checkpoint $model_path/chkpnt_8000.pth
done
...
for data in ${dataset[@]};
do
    model_path=output/hybrid_gs/${data}/hard_${full}
    source_path=datasets/${data}/${full}
    # 在yaml配置文件中恢复了model_path和source_path的默认值，因此这里不需要再传入参数了。
    CUDA_VISIBLE_DEVICES=${device} python3 train.py --config configs/${config}/HybridGS_${Full}_Hard.yaml  --model_path $model_path \
    --source_path $source_path 
    CUDA_VISIBLE_DEVICES=${device} python3 render.py --config configs/${config}/HybridGS_${Full}_Hard.yaml  --model_path $model_path \
    --source_path $source_path --start_checkpoint $model_path/chkpnt_10000.pth
done
...
for data in ${dataset[@]};
do
    model_path=output/hybrid_gs/${data}/soft_${full}
    source_path=datasets/${data}/${full}
    # 在yaml配置文件中恢复了model_path和source_path的默认值，因此这里不需要再传入参数了。
    CUDA_VISIBLE_DEVICES=${device} python3 train.py --config configs/${config}/HybridGS_${Full}_Soft.yaml  --model_path $model_path \
    --source_path $source_path 
    CUDA_VISIBLE_DEVICES=${device} python3 render.py --config configs/${config}/HybridGS_${Full}_Soft.yaml  --model_path $model_path \
    --source_path $source_path --start_checkpoint $model_path/chkpnt_10000.pth
done
