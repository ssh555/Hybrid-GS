...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", num_pts=100_000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False):
    ...
    xyz = np.random.random((init_seed_pts, 3)) * 2.6 - 1.3
    shs = np.random.random((init_seed_pts, 3)) / 255.0
    pcd = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((init_seed_pts, 3)))
